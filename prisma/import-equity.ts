...
async function main() {
    const filePath = path.join(process.cwd(), 'equityinvest.json');
    if (!fs.existsSync(filePath)) {
        console.error('File equityinvest.json not found!');
        return;
    }
    ...
    try {
        ...
        console.log(`Found ${rawQuestions.length} Equity Investments questions to import.`);
        ...
        console.log('Equity Investments import successful!');
    } catch (error) {
        console.error('Failed to parse or import Equity Investments questions:', error);
    }
}
