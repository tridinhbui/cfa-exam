...
function LessonsContent() {
    ...
    const handleSelectBook = (book: Book) => {
        setSelectedBook(book);
        setVisibleReadingsCount(8);
        router.push(`/lessons?bookId=${book.id}`, { scroll: false });
    };

    const handleSelectReading = (reading: Reading) => {
        setSelectedReading(reading);
        if (selectedBook) {
            router.push(`/lessons?bookId=${selectedBook.id}&readingId=${reading.id}`, { scroll: false });
        }
    };
    ...
    const handleBack = () => {
        if (selectedReading) {
            ...
            router.push(`/lessons?bookId=${selectedBook?.id}`, { scroll: false });
        } else {
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground mt-1 text-lg">
                        {selectedReading
                            ? `Mastering ${selectedReading.modules.length} lesson modules`
                            : selectedBook
                                ? `Study ${selectedBook.readings.length} readings from this book`
                                : 'Select a book to start learning the CFA curriculum step-by-step'
                        }
                    </p>
                </div>
            </div>
                ...
                {!selectedBook ? (
                            ...
                            <Card
                                ...
                                className={cn(
                                    ...
                                    book.isLocked
                                        ...
                                        : "hover:border-indigo-500/50 border-border/50"
                                )}
                            >
                                {book.isLocked && (
                    ...
                    <div className="space-y-10">
                        ...
                        >
                            {selectedBook.readings.slice(0, visibleReadingsCount).map((reading: Reading, index: number) => (
                                ...
                                >
                                    <Card className="h-full hover:bg-muted/30 transition-colors border border-border/60 hover:border-indigo-500/40 group cursor-pointer relative overflow-hidden">
                                        <div className="absolute top-0 left-0 w-1 h-full bg-indigo-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                                        <CardContent className="p-5 flex flex-col justify-between h-full">
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-xs font-black text-indigo-500 bg-indigo-500/10 px-2.5 py-1 rounded-md border border-indigo-500/20 shadow-sm shadow-indigo-500/5">
                                                ...
                                                <h4 className="font-bold text-base leading-snug group-hover:text-indigo-500 transition-colors">
                                                    {reading.title}
                                                </h4>
                                            </div>
                                            ...
                                            <div className="mt-4 space-y-3">
                                                <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-muted-foreground px-0.5">
                                                    ...
                                                    <span className={cn(
                                                        "transition-colors",
                                                        reading.modules.filter((m: any) => m.isCompleted).length === reading.modules.length && reading.modules.length > 0
                                                            ? "text-emerald-400"
                                                            : "text-muted-foreground"
                                                    )}>
                                                        {reading.modules.filter((m: any) => m.isCompleted).length}/{reading.modules.length}
                                                    </span>
                                                </div>
                                                <Progress
                                                    value={reading.modules.length > 0 ? (reading.modules.filter((m: any) => m.isCompleted).length / reading.modules.length) * 100 : 0}
                                                    className="h-1.5 bg-muted shadow-inner"
                                                    indicatorClassName="from-emerald-600 to-teal-600"
                                                />
                                            </div>
                                            ...
                                            <div className="mt-4 pt-4 border-t border-border/50 flex items-center justify-between">
                                                ...
                                                <ChevronRight className="h-4 w-4 text-muted-foreground group-hover:text-emerald-500 transition-colors group-hover:translate-x-0.5" />
                                            </div>
                                        </CardContent>
                                    </Card>
                                </motion.div>
                        ...
                        {visibleReadingsCount < selectedBook.readings.length && (
                            <div className="flex justify-center pt-2">
                                <Button
                                    ...
                                    className="px-12 py-6 rounded-2xl border-2 border-indigo-500/20 hover:border-indigo-500/50 hover:bg-indigo-500/5 text-indigo-500 font-bold transition-all shadow-xl shadow-indigo-500/5"
                                >
                                    Load More Readings
                    ...
                    >
                        {selectedReading.modules.map((module: Module, index: number) => (
                            ...
                            >
                                <Card className="hover:bg-muted/30 transition-all border border-border/60 hover:border-emerald-500/40 group cursor-pointer relative overflow-hidden bg-gradient-to-r from-transparent to-transparent hover:to-emerald-500/5">
                                    <CardContent className="p-6 flex items-center gap-6">
                                        <div className={cn(
                                            "flex-shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center border transition-colors relative group",
                                            module.isCompleted
                                                ? "bg-indigo-500/10 border-indigo-500/30 group-hover:bg-indigo-500/20 shadow-inner"
                                                : "bg-emerald-500/10 border-emerald-500/20 group-hover:bg-emerald-500/20"
                                        )}>
                                            <span className={cn(
                                                "text-sm font-black tracking-tighter",
                                                module.isCompleted ? "text-indigo-400" : "text-emerald-500"
                                            )}>
                                                M {module.code}
                                            </span>
                                            {module.isCompleted && (
                                                <div className="absolute -top-1.5 -right-1.5 bg-indigo-500 rounded-full p-1 shadow-lg shadow-indigo-500/30 border-2 border-slate-900">
                                        ...
                                        <div className="flex-grow">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                                                    Lesson Module {module.code}
                                                </span>
                                            </div>
                                            <h4 className="font-extrabold text-lg leading-tight group-hover:text-emerald-400 transition-colors">
                                                {module.title}
                                            </h4>
                                        </div>
