...
export function VaporizeTextCycle({
            ...
            switch (animationState) {
                ...
                case "fadingIn": {
                    ...
                    particlesRef.current.forEach(particle => {
                        ...
                        const colorAttr = particle.color.replace(/[\d.]+\)$/, `${opacity})`);
    ...
    useEffect(() => {
        ...
        if (currentFont !== lastFontRef.current) {
            ...
            const timeoutId = setTimeout(() => {
                cleanup({ canvasRef: canvasRef, particlesRef });
                if (wrapperSize.width > 0 && wrapperSize.height > 0) {
                    renderCanvas({
                        framerProps: { texts, font, color, alignment },
                        canvasRef: canvasRef,
    ...
    return (
        <div ref={wrapperRef} className={className} style={{ ...wrapperStyle, position: 'relative' }}>
...
const cleanup = ({ canvasRef, particlesRef }: { canvasRef: React.RefObject<HTMLCanvasElement | null>; particlesRef: React.MutableRefObject<Particle[]> }) => {
    const canvas = canvasRef.current;
    const ctx = canvas?.getContext("2d");
    if (canvas && ctx) {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
...
const updateParticles = (
    ...
    particles.forEach(particle => {
        const shouldVaporize = direction === "left-to-right"
            ? particle.originalX <= vaporizeX : particle.originalX >= vaporizeX;
        if (shouldVaporize) {
            if (particle.speed === 0) {
...
const renderParticles = (ctx: CanvasRenderingContext2D, particles: Particle[], globalDpr: number) => {
    ...
    particles.forEach(particle => {
        if (particle.opacity > 0) {
            const colorAttr = particle.color.replace(/[\d.]+\)$/, `${particle.opacity})`);
            ctx.fillStyle = colorAttr;
            ctx.fillRect(particle.x / globalDpr, particle.y / globalDpr, 1, 1);
        }
    });
